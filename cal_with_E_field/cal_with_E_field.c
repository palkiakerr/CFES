...
////////////////////////////////////////////////////////////////////////////////////////////
    ...
    gsl_matrix *E_x = gsl_matrix_calloc(dim,dim);
    gsl_matrix *E_y = gsl_matrix_calloc(dim,dim);
    double value;
    //define demensions
  ...
  int count =0;
    ...
    for(int i=1; i< dim-1; i++){
        ...
        element = element + 2;
        //This is needed to jump to next rown in the list 
        for(int j=1; j< dim-1; j++){
        	count++;
            //Used to increment list element
        	//We wish this to loop for dim-2 times as we are only considering the inner matrix(we are removing the 2 end points -e.g. if you have 40 then loop 38)
            value =-(gsl_vector_get(x,(dim+count+1)+element) - gsl_vector_get(x,(dim+count-1)+element))/(2);       
            gsl_matrix_set(E_x,i,j,value);
    ...
    for(int i=1; i< dim-1; i++){
        //question: do we start from matrix element (0,0) or (1,1)
    	element = element + 2;
         //This is needed to jump to next rown in the list 
	        for(int j=1; j< dim-1; j++){
	        	count++;
                 //Used to increment list element
	        	//We wish this to loop for dim-2 times as we are only considering the inner matrix(we are removing the 2 end points -e.g. if you have 40 then loop 38)
	            value = -(gsl_vector_get(x,(dim+count)+(element+dim)) - gsl_vector_get(x,(dim+count)+(element-dim)))/(2);
           		 gsl_matrix_set(E_y,i,j,value);
     ...
     FILE *fp;
   	fp = fopen("E_x.txt", "w");
   	gsl_matrix_fprintf(fp,E_x,"%f");

   	fp = fopen("E_y.txt", "w");
    gsl_matrix_fprintf(fp,E_y,"%f");
    //Print files
